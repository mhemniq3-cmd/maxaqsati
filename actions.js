...
window.checkPasswordAndOpen = async function() {
    ...
    if (enteredHash === targetHash) {
        window.SecurityEngine.resetAttempts();
        if (err) err.classList.add('hidden');
        if (lockoutBanner) lockoutBanner.classList.add('hidden');
        if (window.SoundEngine) window.SoundEngine.playSuccessChime();
        ...
        const failState = window.SecurityEngine.recordFailedAttempt();
        if (failState.locked) {
            if (passInput) passInput.disabled = true;
            if (submitBtn) submitBtn.disabled = true;
            if (err) err.classList.add('hidden');
...
window.saveSettingsFromModal = function() {
    ...
    window.render();
    window.showToast('✅ تم حفظ كافة النسب والإعدادات المشفرة بنجاح');
};
...
// Wire up Action Buttons on Init
document.addEventListener('DOMContentLoaded', () => {
    try {
        ...
        if (btnExportBackup) {
            btnExportBackup.addEventListener('click', () => window.SecurityEngine.exportBackup());
        }
